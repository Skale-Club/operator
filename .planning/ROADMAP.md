...
- [ ] **Phase 42: Google SSO + Team Invites (Admin Auth)** - Google Sign-In for admin dashboard (additional to email/password); allowlist via `org_invites` table; multi-org friendly; trust Google 2FA (no own MFA)
...
**Plans**: 4 plans
- [x] 35-01-PLAN.md — Migration 043: conversations.agent_id ADD COLUMN + backfill + types update + DB push (GATE-01)
- [x] 35-02-PLAN.md — AgentRunOptions: agentId optional + stream flag + channel-defaults resolution + KB unconditional (CHAN-03, GATE-01)
- [x] 35-03-PLAN.md — runAgentStreaming + route.ts cutover (remove tool-fetching block, add maxDuration) + createChatStream shim (CHAN-03, GATE-01)
- [x] 35-04-PLAN.md — GATE-01 test (web-widget-canary.test.ts) + update chat-api.test.ts mock targets + full suite + build gate (GATE-01)
**UI hint**: yes
...
### Phase 42: Google SSO + Team Invites (Admin Auth)
**Goal**: Add Google Sign-In as an additional authentication option for admin dashboard (coexists with existing email/password). Restrict access via per-org allowlist (`org_invites` table) — admin invites email, first OAuth login of invited email auto-creates membership. Multi-org friendly. No own MFA (trust Google 2FA). Source of decisions: `.planning/seeds/SEED-003-google-sso-admin-auth.md`.
**Depends on**: None (independent of v2.0 phases 37-41)
**Requirements**: AUTH-01, AUTH-02, AUTH-03, AUTH-04, AUTH-05, AUTH-06, AUTH-07 (defined inside this phase since SEED-003 didn't add to REQUIREMENTS.md)
**Success Criteria** (what must be TRUE):
  ...
  2. Email NOT in any `org_invites` row is blocked at the OAuth callback with redirect to `/login?error=not_invited` (no `auth.users` row should be created OR if created, no membership is granted)
  3. First OAuth login of an invited email creates the `memberships` row + marks `org_invites.accepted_at` + sets cookie `vo_active_org`

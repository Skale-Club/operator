...
**Plans**: 3 plans
- [x] 69-01-PLAN.md — Pure-function foundation: reserved-keys.ts, serialize.ts, render-config.ts, index.ts (no I/O)
- [x] 69-02-PLAN.md — validate.ts with dynamic zod validation wired into contacts, accounts, and pipeline server actions
- [x] 69-03-PLAN.md — Unit tests (mocked Supabase): unknown key, required, type check, unique_per_org, currency round-trip, serializer
